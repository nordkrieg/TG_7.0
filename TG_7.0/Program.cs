...
namespace TG_7._0;
    internal abstract class Program
    {
        ...
        {
            await Bot.SendPhotoAsync(chatId, "https://sun9-77.userapi.com/impg/as1MA-6kTJiBgNaTzlJchVz9WIdRuTZt9uNJpQ/2kp1pa0vxL4.jpg?size=994x467&quality=96&sign=87102e4153f1c047a2012aa21487f1cb&type=album", cancellationToken: cancellationToken);
        }
        ...
        private static async Task SendSupportInfo(long chatId, CancellationToken cancellationToken)
        {
            ...
            await Bot.SendMessageAsync(chatId, supportInfo, parseMode: ParseMode.MarkdownV2, cancellationToken: cancellationToken);
        }
        ...
        private static IEnumerable<InlineKeyboardButton[]> CreateCalendar(Month mon)
        {
            ...
            calendar[^1][0] = InlineButtonBuilder.SetCallbackData($"{previousmonth}",
                $"month {previousyear} {(ushort)previousmonth}");
            calendar[^1][1] =
        ...
        private static IEnumerable<InlineKeyboardButton[]> CreateCalendar(uint year)
        {
            ...
            keyboard[0] = new[] { InlineButtonBuilder.SetCallbackData($"{year}", $"Year {year}") };
