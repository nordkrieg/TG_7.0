...
internal abstract class SpamBlock
{
    ...
    public static async Task<bool> Task(Message message, CancellationToken cancellationToken, BotClient botClient)
    {
        ...
        if (BannedUserIds.Contains((int)message.Chat.Id))
        {
            ...
            await botClient.SendMessageAsync(message.Chat.Id, "Ваше сообщение было удалено, а вы заблокированы",
        ...
        {
            Console.WriteLine(
                $"Сообщение от замученого пользователя {message.From.Id} не обрабатывается: {message.Text}");
            return true;
        }
        ...
        await botClient.SendPhotoAsync(message.Chat.Id,
            "https://steamuserimages-a.akamaihd.net/ugc/956346433289890009/369C4E7EA8C212D161EDF7840539C0F3F8FFE505/?imw=5000&imh=5000&ima=fit&impolicy=Letterbox&imcolor=%23000000&letterbox=false",
            cancellationToken: cancellationToken);
    ...
    {
        UserBlockedUntil[userId] = DateTime.Now.Add(TimeSpan.FromMinutes(1));
    }
